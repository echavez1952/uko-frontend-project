...
export const MenuSection = () => {
  ...
  const handleSelectProduct = async (product) => {
    ...
    setItems([]); // 🔹 limpiar antes de cargar
    try {
      const res = await axios.get(
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex flex-wrap gap-6 justify-center border-b pb-4 mb-6">
          {components.length === 0 && <p className="text-gray-500">No hay componentes cargados</p>}
          {components.map((comp) => (
            <span
              key={comp._id}
        ...
        {selectedComponent && (
          <div className="mb-8">
            <div className="bg-purple-900 text-white text-center py-2 rounded-lg mb-4">
          ...
          <>
            {items.length === 0 ? (
              <p className="text-center text-gray-600 italic">
                Este producto no tiene ítems disponibles.
              </p>
            ) : (
      ...
      <footer className="bg-purple-900 text-white text-center py-4">
        <p className="text-sm">&copy; {new Date().getFullYear()} Mi Restaurante. Todos los derechos reservados.</p>
      </footer>
    </div>
  );
};
